...
def devide_noise(x):
    ...
    thres = x_fft.abs().median(dim=-1).values
    B, C =  thres.shape
    noise = torch.zeros_like(x_fft,device='cuda')
    for b in range(B):
        for c in range(C):
            noise[b,c,:] = x_fft[b,c,:].where(x_fft[b,c,:].abs() < thres[b,c],torch.tensor(0,device='cuda'))        

    regular = x_fft - noise
    regular = torch.fft.irfft(regular)
    noise = torch.fft.irfft(noise)

    return regular.permute(0,2,1), noise.permute(0,2,1)
